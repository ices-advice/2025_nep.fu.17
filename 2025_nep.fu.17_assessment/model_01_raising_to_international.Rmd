...
* Rasie national lfd to the international landings and calculate fishery summary table.
* Write TAF data tables

Files before (inside "data/intercatch"):

* fu17.int.land.csv
* NepLand.WGNEPH2025.csv
* fu17.nat.qlfd.all.wg2025.csv

Files after (inside "model_01"):

* fu17.international_landings.csv
* fu17.national_landings.csv
* fu17_Int.lfd.png
* fu17_meanld.csv
* fu17_fishery_summary.csv
* fu17_National_landings_qtr.png
* fu17.int.qlfd.csv
...
```{r nat land, echo=TRUE, message=FALSE, fig.height=7, fig.width=12, warning=FALSE, fig.cap="FU17 Irish  Landings (t) by quarter."}
...
aggregate(LiveWt/1000 ~ Year + FU, nat, sum)

nat$Quarter <- as.factor(nat$Quarter)
nat <- nat %>% filter(FU=='17')
  
ggplot(nat, aes(as.factor(Year), LiveWt/1000, fill=factor(Quarter))) + 
...
if (save.plots == T) {
  ggsave("model/model_01_raising_to_international/FU17_National_Landings_qtr.png")
}

 if (save.tables == T) {
   write.csv(nl, "model/model_01_raising_to_international/fu17.national.landings.csv", row.names = F)
   }
...
```{r int land, echo=TRUE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE, fig.cap="FU17 International Landings (t) ."}

int.land <- read.csv("boot/data/intercatch/fu17.int.land.csv")

knitr::kable(int.land , digits=3)

int.land <- gather(int.land,"country", "landings",2:4) 

pi <- ggplot(int.land, aes(Year, landings, fill=country)) + geom_bar(stat="identity", position= "stack") + 
  theme_bw() + ylab("Landings in Tonnes") +
  scale_fill_manual(breaks = c("France", "Ireland", "UK"),
                     values=c("#56B4E9","#009E73", "#CC6666")) +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
  scale_x_continuous(breaks = c(seq(min(int.land$Year), max(int.land$Year), by = 5), max(int.land$Year))) +
  scale_y_continuous(breaks = seq(0, 1500, 250))

pi + theme(axis.text=element_text(size=12),axis.title=element_text(size=14),legend.text = element_text(size = 16),legend.title = element_text(size = 16)) 
...
if (save.plots == T) {
  write.csv(int, "model/model_01_raising_to_international/fu17.international.landings.csv", row.names = F)
}

 if (save.tables == T) {
   ggsave("model/model_01_raising_to_international/FU17_International_Landings.png", width = 12, height = 6, dpi = 300)
   }

```


## Read in quarterly raised data from 01_FU17_Extraction_WGNEPH_2025.rmd
<a href="#top">Back to top</a>

```{r load data, include=FALSE}

qlfd.wg <- read.csv("boot/data/intercatch/fu17.nat.qlfd.all.taf.2025.csv")
```
...
dat$rdis.num <- with(dat, rdis.n.r*rf2)
dat$rlan.num <- with(dat, rlan.n.r*rf2)
dat$rdis.wt <- with(dat, rdis.wt.r*rf2)
dat$rlan.wt <- with(dat, rlan.wt.r*rf2)
...
knitr::kable(check , digits=5)
 ...
 if (save.tables == T) {
   write.csv(dat, "model/model_01_raising_to_international/fu17.int.qlfd.csv",  sep=",", row.names = F)
 }
...
```{r summary plot1, echo=FALSE, warning=FALSE, fig.height= 10, fig.width=8,fig.align='center', fig.cap="FU17 Raised LFD and mean size displayed where black line = mean size in landings and red line = mean size in discards."}
...
vline.data <- rldf %>% group_by(year, sex, type)  %>% summarise(mean=sum(cl.n)/sum(n))
 ...
 if (save.tables == T) {
   write.csv(vline.data, "model/model_01_raising_to_international/fu17_meanld.csv",  sep=",", row.names = F)
 }
...
if (save.plots == T) {
  ggsave("model/model_01_raising_to_international/fu17.Int.lfd.png", width=6)
}
...
if (save.tables == T) {
   write.csv(summ, "model/model_01_raising_to_international/fu17.fishery.summary.csv",  row.names = F)
 }
```
<a href="#top">Back to top</a>
